...
exports.promoteOptimisticLock = promoteOptimisticLock;
...
function addLock(lock, callback) {
    ...
    if (thisLock.waitTime > 0) thisLock.waitTimer = setTimeout(function () {
...
function promoteOptimisticLock(lockUUID) {
    ...
    if (lockQueue.getMode() === 'O') {
        thisLock.mode = 'E';
        var otherOptimisticLocks = lockQueue.queue.filter(function (eleLock) {
            return eleLock.mode === 'O';
        });
        otherOptimisticLocks.forEach(function (eleLock) {
...
function getLocksBy(name, owner) {
    ...
    return eleLocks.map(function (item) {
        return {
...
function _getLockQueue(name, argument) {
    ...
    var lockQueue = LockTable.find(function (LockQueue) {
        if (LockQueue.name === name && _checkArgumentCollision(LockQueue.getArgument(), argument)) return true;
    });
